...
import numpy as np
import time
import quoridor.pathFinder_module.pathFinder as pathFinder
...
class Board:
    ...
    def __init__(self, n):
        ...
        self.n = n * 2 - 1  # board size is self.n x self.n
        self.n_ = n - 1
    ...
    def wall_moves(self, color, pool):
        ...
        for x in range(1, self.n, 2):
            for y in range(0, self.n - 2, 2):
                counter += 1
                if (not block[x][y] and not block[x][y + 2] and (
                        (not block[x - 1][y + 1] or not block[x + 1][y + 1]) or (
                        x - 3 >= 0 and x + 3 < self.n and block[x - 1][y + 1] and block[x - 3][y + 1] and block[x + 1][
                    y + 1] and block[x + 3][y + 1]))):
        ...
        for x in range(2, self.n, 2):
            for y in range(1, self.n, 2):
                counter += 1
                if (not block[x][y] and not block[x - 2][y] and (
                        (not block[x - 1][y - 1] or not block[x - 1][y + 1]) or (
                        y - 3 >= 3 and y + 3 < self.n and block[x - 1][y - 1] and block[x - 1][y - 3] and block[x - 1][
                    y + 1] and block[x - 1][y + 3]))):
        ...
        if np.sum(moves_boolean) > len(moves_boolean) / 3:
            ...
            if parent != 0 and parent_ != 0:
                valid_walls = self.walls_not_in_path(parent, tail) & self.walls_not_in_path(parent_,
                                                                                            tail_) & moves_boolean
    ...
    def is_wall_legal(self, move):
        if move is None or move == [0]: return 0
        if move == [1]:
            return 1
        w, (x, y), block_ = move
    ...
    def destination_cells_from(self, x, y, block, visited, parent, cells):
        # cells = []
        if parent is None:  # minor optimization
